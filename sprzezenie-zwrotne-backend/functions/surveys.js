...
const getCoursesAvailableToSurvey = async function (token, userdata) {
    ...
    for (let i = 0; i < currentCourseEditions.length; i++) {
        url = 'https://usosapps.umk.pl/services/courses/course_edition';
        returnObject = await got(url, {
            headers: oauth.toHeader(oauth.authorize({ url: url, method: 'POST' }, token)),
            searchParams: {
                course_id: currentCourseEditions[i].courseId,
                term_id: currentCourseEditions[i].termId,
    ...
    let obKeys = Object.keys(returnCoursesBeforeProcessing);
    for (var i = 0; i < obKeys.length; i++) {
        // returnCourses.push(returnCoursesBeforeProcessing[obKeys[i]]);
        returnCourses.push({
            course_name: returnCoursesBeforeProcessing[obKeys[i]].course_name,
            term_id: returnCoursesBeforeProcessing[obKeys[i]].term_id,
            course_id: returnCoursesBeforeProcessing[obKeys[i]].course_id,
            course_unit_id: returnCoursesBeforeProcessing[obKeys[i]].id + "",
            class_type_id: returnCoursesBeforeProcessing[obKeys[i]].classtype_id,
        });
    }
...
const getAvailableSurveys = async function (mongoInfo, token, userdata) {
    ...
    if (data.course_editions[wholeYearTerm])
        currentCourses = data.course_editions['2020/21Z'].concat(data.course_editions[wholeYearTerm]);
    else
        currentCourses = data.course_editions['2020/21Z'];
        ...
        .then(function (result) {
            if (result) {
                result.forEach((element) => {
                    mySurvey.surveyAnswers.push({
                        answers: element.answers, surveyComment: element.surveyComment,
